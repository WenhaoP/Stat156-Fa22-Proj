...
X = merged_df[,c('TOT_100_G','Y_0','polity2l','ethfrac','relfrac','Oil','lpopl1','lmtnest')]
merged_df %>% select(contains('Iccyear'))
m2 = lm(formula = GDP_G ~ GPCP_G + GPCP_G_L + X + Iccyear*, data = merged_df)
m2.se = sqrt(diag(sandwich::vcovCL(m1, cluster = ~ccode,type = 'HC1')))

stargazer(m1, se = list(m1.se), type = 'text')
