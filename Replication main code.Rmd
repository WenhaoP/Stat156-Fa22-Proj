...
## Data Cleaning for the Civil Conflict Data
...
# write_csv(x = master_df, file = 'master_df.csv')
...
# IV 2SLS 5:
# stage 1:
m5formula1 = reformulate(c(grep('Iccyear',x = names(master_df),value = T),'GPCP_G',X))
# stage 2: 
m5formula2 = reformulate(c(grep('Iccyear',x = names(master_df),value = T),X,'GDP_G_L'),response = 'ANY_PRIO')
...
# IV 2SLS 6:
# stage 1:
m6formula1 = reformulate(c(grep('Iccyear|Iccode',x = names(master_df),value = T),'GPCP_G'))
# stage 2: 
m6formula2 = reformulate(c(grep('Iccyear|Iccode',x = names(master_df),value = T),'GDP_G_L'),response = 'ANY_PRIO')
...
# IV 2SLS 7:
# stage 1:
m7formula1 = reformulate(c(grep('Iccyear|Iccode',x = names(master_df),value = T),'GPCP_G'))
# stage 2: 
m7formula2 = reformulate(c(grep('Iccyear|Iccode',x = names(master_df),value = T),'GDP_G_L'),response = 'WAR_PRIO')
...
```{r} 
# (Fieller-)Anderson-Rubin test
# construct confidence interval for IV using the duality between hypothesis testing and confidence interval


get_AR_pvals = function(y,z,d,with_cov=T, betas = seq(-10,10,0.1)){
  .data  = master_df
...
betas = seq(-10,10,0.01)
AR_pvals.Y1.z1.d1 = get_AR_pvals(y = 'ANY_PRIO', z  = 'GPCP_G', d = 'GDP_G_L', with_cov = T, betas = betas)
...
plot(AR_pvals.Y1.z1.d1 ~ betas, type = "l",
     ...
     ylab = "p-value",
     main = "Anderson-Rubin interval for the effect of GDP Growth on Civil Conflict")
