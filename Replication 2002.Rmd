...
```{r}
main =read_csv("armed.csv")

#main =readxl::read_excel("armed.xls")
africa = read_csv('cow_code.csv')
# raw = haven::read_dta('dataverse_files-4/mss_repdata.dta')
monadic = main %>% 
  select(Type, Intensity, YEAR, COW_B, Location) %>%
  pivot_wider(id_cols = c(YEAR,COW_B), names_from = Type,values_from = Intensity, values_fn = max, names_prefix = 'Type', values_fill = 0) 



monadic_africa_conflict = monadic %>% 
  filter(COW_B %in% africa$CCode) %>% 
  mutate(COW_B = as.numeric(COW_B))

# Generate an "empty" table
monadic_africa_empty = map_df(1:nrow(africa), function(i){
    bind_cols(Location = africa$StateNme[i], 
        COW_B = africa$CCode[i], 
        YEAR = 1980:2000) 
})
...
merged_df = inner_join(x = df, y = monadic_africa_all,by = c('year' = 'YEAR', 'ccode' = 'COW_B'))
